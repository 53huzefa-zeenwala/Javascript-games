...
window.addEventListener("load", function () {
  ...
  class Egg {
    ...
    update(deltaTime) {
      ...
      if (this.hatchTimer > this.hatchInterval || this.collisionY < this.game.topMargin) {
        this.game.hatcheling.push(new Larva(this.game, this.collisionX, this.collisionY))
  ...
  class Larva {
    ...
    update() {
      ...
      if (this.collisionY < this.game.topMargin - 60) {
        this.markedForDeletion = true  
        this.game.removeGameObject()
      ...
      this.game.enemies.forEach(enemy => {
        if (this.game.checkCollision(this, enemy)[0]) {
          this.markedForDeletion = true
          this.game.removeGameObject()
          if(!this.game.gameOver) this.game.lostHatcheling++
          for (let i = 0; i < 6; i++) {
  ...
  class Game {
    ...
    render(context, deltaTime) {
      if (this.timer > this.interval) {
        ...
        this.gameObjects = [...this.eggs, ...this.obstacles, this.player, ...this.enemies, ...this.hatcheling, ...this.particles]
      ...
      context.fillText('lost: ' + this.lostHatcheling, 25, 100)
      context.restore()
    ...
    removeGameObject () {
      ...
      this.hatcheling = this.hatcheling.filter(object => !object.markedForDeletion)
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === " "){ 
      ...
      if (!game.gamePause) animate(timeStampExtra) 
    }
  });
  animate(0);
});
